...
| **Request Type** | New Project (Greenfield ハッカソンプロダクト) |
...
### ハッカソン文脈
- **主催**: AWS Summit Japan 2026 ハッカソン
- **テーマ**: 「人をダメにするサービス」
- **書類審査締切**: 2026-05-10 (Inception フェーズ成果物が審査対象)
- **フェーズ 2**: 予選通過後の 1 ヶ月で開発
- **主催規約**: AWS サービスのみで構築 (NFR-DAT-03)
...
4. **ジャッジと悪魔がデータを根拠に対話する形でメッセージが表示される**
5. ユーザーが「入る」「サボる」を選択
...
| FR-11 | **ダラけ感のある UI 演出 (新規)** | キャラがソファに沈む、ぬるっと動く、ゆるいフォント等 |
...
| NFR-DAT-03 | AWS サービスのみで構築 (主催規約準拠) |
...
intent.md「技術スタック」表を本ファイル内に直接記述。AWS 主催ハッカソン前提のため、AWS サービスを基本とする。
...
| AWS 環境準備 (確認済み: Q5) | **書類審査段階では「想定環境」として記載** | AWS アカウント・Bedrock Claude モデルアクセス申請 (**Claude Sonnet 4.6 第一候補 + Claude Opus 4.7 拡張可能** / ap-northeast-1) は **予選通過後の最初の Bolt** で実施 (R8/R9 と整合)。予選通過直後の最優先タスクとして明示 |
...
| R8 | AWS アカウント・権限準備の遅延 | **高** | 最初の Bolt で完了させる (Q5 回答と整合: 予選通過後の最優先タスク) |
...
| R11 | **倫理的リスク**: 「サボりを助長する」批判への備え | 中 | **3 段構えで対応**: (a) **ユーザーへのコンセプト明示** (US-5.5 オンボーディング: 「ふざけたアプリ」「真剣な健康相談には使わないでください」を初回起動で明示・同意必須) / (b) **システム側で迷惑ライン設計** (US-2.4 動的トーンシフト: 周囲迷惑リスク高時は悪魔も柔らかく入浴を推奨) / (c) **予選通過後の最初の Bolt で法務観点レビュー** (US-5.5 文言の法的妥当性を専門家レビュー / R11 対策の最終仕上げ)。デモ説明での前置きも併用 |
...
- 書類審査では「想定環境」として記載し、実際の準備は予選通過後に行う
- AWS アカウント: 「予選通過後の最初の Bolt で準備」と明記 (R8)
- Bedrock Claude モデルアクセス: 「ap-northeast-1 (Tokyo) リージョンで Claude モデルアクセスを予選通過後の最初の Bolt で申請・承認取得」と明記 (R9)
- 申請承認に時間がかかる可能性 (R9) は予選通過直後の **最優先タスク** として明示
