...
| 層 | サービス | 備考 |
|---|---|---|
| LLM | Amazon Bedrock (Claude 3.5 Sonnet 等) | 対話生成・綺麗度定性判定 |
...
| AWS 環境準備 (確認済み: Q5) | **書類審査段階では「想定環境」として記載** | AWS アカウント・Bedrock Claude モデルアクセス申請 (ap-northeast-1) は **予選通過後の最初の Bolt** で実施 (R8/R9 と整合)。予選通過直後の最優先タスクとして明示 |
...
| R9 | Bedrock のモデルアクセス申請 | 中 | 最初の Bolt で一緒に対応 (Q5 回答と整合: ap-northeast-1 で Claude モデルアクセス申請) |
